...
function addItemToCategory(items, urlPrefix, addFunction) {
    items.forEach(item => {
        addFunction(item, `${urlPrefix}${item}`);
    });
}

Object.entries(addFunctions).forEach(([key, addFunction]) => {
    if(categories[key]) {
        addItemToCategory(categories[key], "index.html?search=", addFunction);
    }
}); 